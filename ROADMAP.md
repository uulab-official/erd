...
- [x] Canvas Entity 노드/Relationship 엣지 실제 렌더링 — `packages/canvas`의 `EntityNode`(로지컬/피지컬 이름, PK/FK 뱃지가 붙은 attribute 목록, 4방향 Handle)가 기존 "라벨만 있는 플레이스홀더 박스"를 대체. 엣지는 `Relationship.kind`에 따라 실선(identifying)/점선(non-identifying), 화살표 마커, `name (cardinality)` 라벨로 스타일링 — [PR #20](https://github.com/uulab-official/erd/pull/20). 캔버스에서 드래그로 Relationship을 생성하는 것(Handle→Handle 연결, `onConnect`)은 여전히 안 됨 — Handle은 시각적 연결점일 뿐이고 실제 Relationship 생성은 아직 Operation을 직접 호출해야 한다.
